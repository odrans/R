...
map.plot <- function(Lat,Lon,Prod,proj="eck4",sampling="cubic",interp=TRUE,overwrite=TRUE,dir.GTiff=".",color="",ncol=64,box.size=2,postprocess=function(x){return(x)},...) {
    ...
    sdf.proj[[name.prod]] <- replace(sdf.proj[[name.prod]],sdf.proj[[name.prod]]<zlim[1],zlim[1])
    sdf.proj[[name.prod]] <- replace(sdf.proj[[name.prod]],sdf.proj[[name.prod]]>zlim[2],zlim[2])
